...
# localizationtool/views.py
# FIXED VERSION — Added missing polib import + stable editing (January 15, 2026)
...
def localize_tool_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            original_pot_name = pot_file.name
            pot_path = os.path.join(project_dir, original_pot_name)
            with open(pot_path, 'wb') as f:
                for chunk in pot_file.chunks():
                    f.write(chunk)
...
def edit_language_version(request, folder_name, lang_code, version):
    ...
    po = polib.pofile(po_path)  # Now works because polib is imported
    entries = []
    for entry in po:
        if entry.msgid:
...
def save_translation_version(request, folder_name, lang_code, version):
    ...
    for entry in po:
        if entry.msgid:
            safe_key = f"trans_{re.sub(r'[^a-zA-Z0-9_]', '_', entry.msgid)[:100]}"
            if safe_key in request.POST:
                new_text = request.POST[safe_key].strip()
                if entry.msgstr != new_text:
                    entry.msgstr = new_text
                    if 'fuzzy' in entry.flags:
                        entry.flags.remove('fuzzy')
    ...
    if updated > 0:
        po.save(po_path)
        po.save_as_mofile(po_path.replace('.po', '.mo'))
        messages.success(request, f"{updated} translations updated successfully!")
    else:
        messages.info(request, "No changes detected. Make sure you edited something and submitted.")
