...
# localizationtool/localization_logic.py
# FINAL FIXED VERSION — Placeholder + Copyright Protection (Feb 23, 2026)
...
class ColabLocalizationTool:
    ...
    def _should_skip_translation(self, original: str) -> bool:
        ...
        for protected in self.PROTECTED_STRINGS:
            if protected in clean or clean in protected:
                return True

        if "©" in clean or "&copy;" in clean or "{year}" in clean and "Copyright" in clean:
            return True

        if "One thought on" in clean and ("%1$s" in clean or "“" in clean or "”" in clean):
            return True
    ...
    # STRONGER PLACEHOLDER PRESERVATION
    def _preserve_placeholders(self, original: str, translated: str) -> str:
        orig_ph = self.placeholder_regex.findall(original)
        if not orig_ph:
            return self._sanitize_for_php(translated)
        ...
        temp = translated
        for ph in orig_ph:
            temp = temp.replace("%s", ph, 1)   # Extra safety for %s
            temp = temp.replace("%d", ph, 1)
        temp = self.placeholder_regex.sub("<<PH>>", temp)
        for ph in orig_ph:
            temp = temp.replace("<<PH>>", ph, 1)
        return self._sanitize_for_php(temp)
    ...
    def _validate_po_file(self, po_path: str) -> List[str]:
        ...
        try:
            ...
            for entry in po:
                ...
                # Ignore valid entities
                if re.search(r'&[a-zA-Z0-9]+(?![;])', entry.msgstr):
                    if not any(ent in entry.msgstr for ent in ['&copy;', '&reg;', '&trade;', '&euro;', '&nbsp;', '&ldquo;', '&rdquo;', '&lsquo;', '&rsquo;']):
                        msg = f"Broken HTML entity → {entry.msgid[:60]}..."
                        warnings.append(msg)
                        print(f"   ⚠ {msg}")
    ...
    # (I kept all other methods exactly as you had them)
    ...
    def _process_translation(self, memory: Dict, pot_entry: polib.POEntry, glossary_lookup: Dict, existing_po_lookup: Dict,
        ...
        # Always try placeholder preservation for any string with %
        if self.placeholder_regex.search(msgid) or '%' in msgid:
            self._counts["placeholder_preserved"] += 1
        ...
        if self._should_skip_translation(msgid):
            self._counts["protected"] += 1
            return msgid, "Protected String"
