...
class ColabLocalizationTool:
    ...
    def _validate_po_file(self, po_path: str):
        """Loco Translate style format checker"""
        warnings = 0
        try:
            ...
            for entry in po:
                ...
                if len(orig_ph) != len(trans_ph):
                    print(f"   ⚠ Validation Warning: Placeholder mismatch → {entry.msgid[:60]}...")
                    warnings += 1
                if re.search(r'%(?!\d*\$?[sdifuxXeEgGcCr])', entry.msgstr):
                    print(f"   ⚠ Validation Warning: Unescaped '%' → {entry.msgid[:60]}...")
                    warnings += 1
                if re.search(r'&[a-zA-Z0-9]+(?![;])', entry.msgstr):
                    print(f"   ⚠ Validation Warning: Broken HTML entity → {entry.msgid[:60]}...")
                    warnings += 1
